...
internal class MyImage : IAddChild
{
    ...
    public MyImage(LinkInline linkInline, Uri uri, MarkdownConfig config)
    {
        ...
        Init();
        var size = MarkdownTextBlockExtensions.GetMarkdownImageSize(linkInline);
        if (size.Width != 0)
        {
...
#pragma warning restore CS8600 // Converting null literal or possible null value to non-nullable type.
            ...
            {
                _image.WidthRequest = _precedentWidth;
            }
            if (_precedentHeight != 0)
            {
                _image.HeightRequest = _precedentHeight;
            }
        }
        catch (Exception) { }
    }
